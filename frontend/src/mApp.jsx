...
export default function NeXaQuant() {
  ...
  const analyzeGuru = async () => {
    ...
    try {
      ...
      // Calculate portfolio value based on average metrics
          ...
          <div className="space-y-6">
            ...
            {showMarketplace && marketplaceResults && (
              <div className="bg-black/30 rounded-xl p-6 border border-purple-500/20 backdrop-blur-sm animate-fadeIn">
                ...
                <div className="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 rounded-lg p-4 mb-6 border border-blue-500/30">
                  <p className="font-semibold text-blue-300 mb-2">Summary:</p>
                  <p className="text-white">{marketplaceResults.summary}</p>
                </div>
                  ...
                  {marketplaceResults.all_negotiations.map((neg, idx) => (
                    <div key={idx} className="bg-black/50 rounded-lg p-4 border border-purple-500/20">
                      <div className="flex justify-between items-center mb-2">
                        <span className="font-semibold text-purple-300">{neg.seller_id} ({neg.personality})</span>
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                          neg.outcome === 'deal_accepted' ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'
                        }`}>
                          {neg.outcome.replace(/_/g, ' ')}
                        </span>
                      </div>
                      <p className="text-sm text-purple-200">Min Price: ${neg.min_price.toFixed(2)}</p>
                      {neg.final_price && (
                        <p className="text-sm text-green-400 font-semibold mt-1">Final Price: ${neg.final_price.toFixed(2)}</p>
                      )}
                      <details className="mt-2">
                        ...
                        <div className="mt-2 space-y-1 text-xs text-purple-200 ml-4">
                          {neg.rounds.slice(0, 5).map((round, ridx) => (
                            <div key={ridx} className="py-1 border-l-2 border-purple-500/30 pl-2">
                              Round {round.round}: {round.buyer_action || round.seller_action}
                            </div>
                          ))}
                        </div>
                      </details>
              ...
              <div className="bg-black/30 rounded-xl p-6 border border-purple-500/20 backdrop-blur-sm animate-fadeIn">
                ...
                <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-lg p-4 border border-purple-500/30">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-purple-200">Period Return:</span>
